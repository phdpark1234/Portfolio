...
function prdInit(data) {
    var cate = data.cate;
    var arr = [data.all.latest, data.all.top, data.all.top, data.all.best];
    /*
    log("data =>",data);
   ...
   for(var i in arr) {
       ...
       for(var j in arr[i]) {
            ...
            html += '<div class="icon_like prd_icon aniset" data-over="bottomShow2" data-out="bottomHide2" data-speed="0.3s" data-delay="0.2s"></i></div>';
            html += '<li>'+v.tit+'</li>';
            html += '<li>';
            for(var k=1; k<=5; k++) {
